...
<body class="scroll-smooth">
            ...
            <ul class="space-y-2 font-medium">
                ...
                <li class="group relative px-6 py-3">
                    ...
                    <a href="./anggota.php" class="flex items-center text-gray-900 rounded-lg">
                        ...
                        <span class="text-base text-gray-900 font-semibold ml-4">Anggota</span>
                    </a>
                </li>
                <li class="group relative px-6 py-3">
                    <span class="absolute inset-y-0 translate-x-[-28px] w-1 bg-[#2363DE] rounded-tr-lg rounded-br-lg group-hover:translate-x-[-24px] transition-transform"></span>
    ...
    <div class="bg-[#f9fafb] p-6 sm:ml-64 h-screen">
        <div class="flex items-center justify-between mb-8">
            <h1 class="text-3xl font-semibold text-black">Data Anggota</h1>
                        ...
                        <table class="w-full whitespace-no-wrap">
                            <thead>
                                <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b">
                                    <th class="pl-6 pr-4 py-3">Kode Anggota</th>
                                ...
                                <?php else : ?>
                                    <?php foreach ($members as $member) : ?>
                                        <tr class="bg-white border-b">
                                            ...
                                            <td class="pl-6 pr-4 py-3 text-black flex justify-center items-center">
                                                <label for="edit<?= $member["kode_anggota"] ?>" data-modal-target="edit<?= $member["kode_anggota"] ?>-modal" data-modal-toggle="edit<?= $member["kode_anggota"] ?>-modal" class="cursor-pointer text-sm text-blue-600">
                                                    Edit
                                                </label>
                                                <p class="mx-2">|</p>
                                                <label for="hapus<?= $member["kode_anggota"] ?>" data-modal-target="hapus<?= $member["kode_anggota"] ?>-modal" data-modal-toggle="hapus<?= $member["kode_anggota"] ?>-modal" class="cursor-pointer text-sm text-red-600">
                                                    Hapus
                                                </label>

                                                <input type="checkbox" id="edit<?= $member["kode_anggota"] ?>" class="modal-toggle" />
                                                <label for="edit<?= $member["kode_anggota"] ?>" class="modal cursor-pointer">
                                                    <label class="modal-box relative bg-white w-[27rem] p-8" for="">
                                                        <label for="edit<?= $member["kode_anggota"] ?>" class="btn btn-sm btn-circle absolute right-4 top-4 bg-[#e1e1e1] text-[#6B7280] border-0 hover:text-[#6B7280] hover:bg-[#e1e1e1]"><i class='bx bx-x text-2xl'></i></label>
                                                        ...
                                                        <form class="space-y-3" action="./controller/anggota.php" method="POST">
                                                            <input name="kode_anggota" type="text" value="<?= $member["kode_anggota"] ?>" hidden>
                                                            <div class="relative mb-3 h-[50px]">
                                                                <input id="nama" class="form__input" type="text" name="nama" autocomplete="off" placeholder=" " value="<?= $member["nama"] ?>" required />
                                                                <label class="form__label" for="nama">Nama</label>
                                                            </div>
                                                            <div class="relative mb-3 h-[50px]">
                                                                <input id="email" class="form__input" type="text" name="email" autocomplete="off" placeholder=" " value="<?= $member["email"] ?>" required />
                                                                <label class="form__label" for="email">Email</label>
                                                            </div>
                                                            <div class="relative mb-3 h-[50px]">
                                                                <input id="no_hp" class="form__input" type="text" name="no_hp" autocomplete="off" placeholder=" " value="<?= $member["no_hp"] ?>" required />
                                                                <label class="form__label" for="no_hp">No HP</label>
                                                            </div>
                                                            <button type="submit" name="edit_anggota" class="w-full inline-block bg-[#2363DE] text-white px-4 py-2 rounded mt-4">Edit</button>
                                                        </form>
                                                    </label>
                                                </label>

                                                <input type="checkbox" id="hapus<?= $member["kode_anggota"] ?>" class="modal-toggle" />
                                                <label for="hapus<?= $member["kode_anggota"] ?>" class="modal cursor-pointer">
                                                    <label class="modal-box relative bg-white w-[27rem] p-8" for="">
                                                        <label for="hapus<?= $member["kode_anggota"] ?>" class="btn btn-sm btn-circle absolute right-4 top-4 bg-[#e1e1e1] text-[#6B7280] border-0 hover:text-[#6B7280] hover:bg-[#e1e1e1]"><i class='bx bx-x text-2xl'></i></label>
                                                        <h3 class="font-bold text-2xl text-[#2d333a] text-center mb-6 mt-2">Apakah Anda Yakin Ingin Menghapus <?= $member["nama"] ?>?</h3>
                                                        <div class="flex justify-center gap-x-2">
                                                            <label for="hapus<?= $member["kode_anggota"] ?>" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-md border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 transition duration-300 cursor-pointer">Tidak</label>
                                                            <a href="./controller/anggota.php?kode_anggota=<?= $member['kode_anggota'] ?>" class="text-white bg-[#2363de] hover:bg-[#0d46b5] focus:ring-4 focus:outline-none focus:ring-[#0d46b5] font-medium rounded-md text-sm inline-flex items-center px-5 py-2.5 text-center mr-2 transition duration-300">
                                                                Ya, saya yakin.
                                                            </a>
                                                        </div>
                                                    </label>
    ...
    <label for="peminjaman" class="modal cursor-pointer">
        <label class="modal-box relative bg-white w-[27rem] p-8" for="">
            ...
            <h3 class="font-bold text-2xl text-[#2d333a] text-center mb-1 mt-2">Form Tambah Anggota</h3>
            ...
            <form class="space-y-3" action="./controller/anggota.php" method="POST">
                ...
                <button type="submit" name="tambah_anggota" class="w-full inline-block bg-[#2363DE] text-white px-4 py-2 rounded mt-4">Tambah</button>
            </form>
        </label>
    </label>
